...
func (c *Cache) Get(key interface{ ToString(str string) string }, strstr string, opts ...OpOption) (interface{}, error, bool) {
	...
	if c.conf.singleflight && conf.singleflight {
		data, err = c.groupOf(key, strstr).Call(conf.ctx, key, strstr, conf.onMissed)
	} else {
...
// DeleteFollow
// 如果不存在key，则不会发生任何事情;否则删除key
func (c *Cache) DeleteFollow(ctx context.Context, key interface{ ToString(str string) string }, strstr string) error {
	pairUserId := PairInt64(key.(StringPairInt64))
	//TODO 调用数据库更新操作
	err := db.Follow(ctx, pairUserId.UserId1, pairUserId.UserId2)
	if err != nil {
		return err
	}

	key1 := strconv.FormatInt(pairUserId.UserId1, 10)
	key2 := strconv.FormatInt(pairUserId.UserId2, 10)
	keykey1 := StringInt64(pairUserId.UserId1)
	keykey2 := StringInt64(pairUserId.UserId2)

	// 删除UserId1的关注列表
	strstr = "Follows"
	tmpkey1 := key1 + strstr
	c.segmentOf(keykey1, strstr).delete(ctx, tmpkey1)
	// 删除UserId2的关注列表
	tmpkey2 := key2 + strstr
	c.segmentOf(keykey2, strstr).delete(ctx, tmpkey2)
	...
	tmp1 := strconv.FormatInt(pairUserId.UserId1, 10)
	tmp2 := strconv.FormatInt(pairUserId.UserId2, 10)
	strstr = "IsFollowed"

	key3 := tmp1 + strstr + tmp2
	c.segmentOf(key, strstr).delete(ctx, key3)

	return err
	//c.groupOf(key, strstr).Delete(key, strstr)
}

// DeleteUnFollow
// 如果不存在key，则不会发生任何事情;否则删除key
func (c *Cache) DeleteUnFollow(ctx context.Context, key interface{ ToString(str string) string }, strstr string) error {
	pairUserId := PairInt64(key.(StringPairInt64))
	//TODO 调用数据库更新操作
	// 可以同时锁住三块
	err := db.Unfollow(ctx, pairUserId.UserId1, pairUserId.UserId2)
	if err != nil {
		return err
	}
